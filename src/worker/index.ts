...
app.post("/api/resume/generate", zValidator("json", CreateResumeSchema), async (c) => {
  try {
    ...
    const genAI = getGeminiClient(c.env);
...
Please format this as a professional resume with clear sections and compelling language that highlights achievements and impact.`;
    ...
    const aiResult = await model.generateContent(`You are a professional resume writer with expertise in creating compelling resumes that get results. Format the resume with clear sections and use action verbs and quantifiable achievements where possible.

${prompt}`);

    const resumeContent = aiResult.response.text();
...
Create a personalized cover letter that highlights relevant experience, shows enthusiasm for the role, and demonstrates knowledge of the company. The letter should be professional, concise, and compelling.`;
    ...
    const aiResult = await model.generateContent(`You are an expert career coach and professional writer specializing in creating compelling cover letters that get interviews. Write personalized, engaging cover letters that highlight the candidate's strengths and fit for the role.

${prompt}`);

    const coverLetterContent = aiResult.response.text();
...
app.post("/api/coaching/chat", zValidator("json", CoachingMessageSchema), async (c) => {
  try {
    ...
    const aiResult = await model.generateContent(`You are Coach Leo, an expert career coach with years of experience helping professionals advance their careers. You provide personalized advice on career development, job search strategies, interview preparation, salary negotiation, and professional growth. Be supportive, actionable, and encouraging in your responses.
...
Format your response as JSON with the following structure:
{
  ...
  "optimizedResume": "full optimized resume text"
    ...
    const aiResult = await model.generateContent(`You are an expert ATS and resume optimization specialist. Analyze resumes for keyword density, ATS compatibility, and provide actionable improvements. Always respond with valid JSON.
...
${prompt}`);
    ...
    try {
      ...
      return c.json({
        score: 75,
        scoreDescription: "Analysis completed with moderate compatibility",
        missingKeywords: ["analysis", "optimization"],
        foundKeywords: ["experience", "skills"],
...
Format as JSON:
{
  ...
  "contentStrategy": ["strategy1", "strategy2"]
    ...
    const aiResult = await model.generateContent(`You are a LinkedIn optimization expert and personal branding specialist. Create compelling, professional profiles that attract recruiters and opportunities. Always respond with valid JSON.
...
${prompt}`);
    ...
    try {
      ...
      return c.json({
        ...
        optimizedSummary: "Optimized summary would appear here with relevant keywords and compelling narrative.",
        suggestedSkills: ["Leadership", "Strategy", "Analytics"],
        recommendations: ["Add professional photo", "Engage with industry content"],
        contentStrategy: ["Share industry insights", "Comment on thought leader posts"]
      });
    }
  } catch (error) {
...
Return as a JSON array of questions: ["question1", "question2", ...]`;
    ...
    const aiResult = await model.generateContent(`You are an expert interview coach and HR professional. Create realistic, challenging interview questions that help candidates prepare effectively. Always respond with valid JSON.
...
app.post("/api/interview/next-question", async (c) => {
  try {
    ...
    if (nextIndex >= questions.length) {
      return c.json({ 
        question: "Interview completed! Great job practicing.",
        isComplete: true 
      });
    }
